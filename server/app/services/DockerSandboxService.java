...
/**
 * Real {@link SandboxService} implementation that launches CiviForm containers via the Docker
 * socket. Replaces {@link InMemorySandboxService} for Sprint 1.
 *
 * <p>Sprint 2 note: replace with {@code EcsFargateSandboxService}. The interface contract is
 * identical — only this class changes.
 */
@Singleton
public class DockerSandboxService implements SandboxService {
  ...
  public DockerSandboxService(
      SandboxRepository repository, Database db, WSClient ws, Config config) {
   ...
   *
   * <p>Bypasses {@link #buildDockerClient} entirely, avoiding the Java constructor-ordering pitfall
   * where calling an overridable method from a parent constructor means the subclass override runs
   * before the subclass's own fields are initialised.
   */
   ...
   *
   * <p>Protected and non-final so tests can subclass and return a mock client without requiring a
   * real Docker socket to be present in the CI environment.
   *
  ...
  /**
   * Creates a sandbox synchronously up to DB persistence, then launches the Docker container
   * asynchronously. Returns immediately with PROVISIONING status. The PIN is available in the
    ...
    CompletableFuture.runAsync(
        () -> {
          try {
            ...
            log.info(
                "[{}] Health check passed, waiting {}ms for DB migrations",
                id,
  ...
  public CompletionStage<Optional<SandboxInstance>> validatePin(String sandboxId, String pin) {
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          boolean matches = MessageDigest.isEqual(sandbox.getPin().getBytes(), pin.getBytes());
        ...
        dockerClient
            ...
            .withHostConfig(
                HostConfig.newHostConfig()
                    ...
                    // On Mac this is a no-op; on Linux it maps to the bridge gateway (e.g.
                    // 172.17.0.1)
                    .withExtraHosts("host.docker.internal:host-gateway", "dev-oidc:host-gateway"))
            .withEnv(
